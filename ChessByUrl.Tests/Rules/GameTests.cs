...
namespace ChessByUrl.Tests.Rules
{
    ...
    public class GameTests
    {
        ...
        public void GetLegalMovesFromSquare_ReturnsFilteredMoves()
        {
            ...
            CollectionAssert.AreEqual(new[] { new Move(new Coords(1, 1), new Coords(4, 4)) }, moves.ToArray());
            moves = fakes.Game.GetLegalMovesFromSquare(new Coords(2, 2));
            CollectionAssert.AreEqual(new[] { new Move(new Coords(2, 2), new Coords(4, 4)) }, moves.ToArray());
            moves = fakes.Game.GetLegalMovesFromSquare(new Coords(3, 3));
            CollectionAssert.AreEqual(Array.Empty<Move>(), moves.ToArray());
        }
        ...
        public void GetThreats_ReturnsUnfilteredThreats()
        {
            ...
            CollectionAssert.AreEqual(
                new[]
                {
                    new Move(new Coords(1, 1), new Coords(4, 4)),
                    new Move(new Coords(2, 2), new Coords(4, 4)),
                    new Move(new Coords(3, 3), new Coords(4, 4))
                }, threats);
        }
        ...
        public void ApplyMove_ReturnsAppliedAndAdjustedBoard()
        {
            ...
            var move = new Move(new Coords(1, 1), new Coords(4, 4));
        ...
        {
            public IEnumerable<Move> GetLegalMovesFrom(Game game, Coords from, PieceType fromPiece) => new[]
            {
                new Move(from, new Coords(4, 4))
            };
